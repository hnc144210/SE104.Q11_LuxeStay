...
exports.updateRental = async (req, res) => {
  try {
    ...
    if (!new_end_date) return res.status(400).json({ success: false, message: "Thiếu ngày kết thúc mới" });
    ...
    if (findError || !rental) return res.status(404).json({ success: false, message: "Không tìm thấy phiếu thuê" });

    if (rental.status !== 'active') return res.status(400).json({ success: false, message: "Chỉ gia hạn được phiếu đang hoạt động" });
    ...
    const { data: conflicts } = await supabase
      ...
      .or(`and(check_in_date.lte.${new_end_date},check_in_date.gte.${rental.end_date})`);

    if (conflicts && conflicts.length > 0) {
      return res.status(400).json({ success: false, message: "Phòng bị vướng lịch đặt trong khoảng gia hạn" });
    }
    ...
    return res.json({ success: true, message: "Gia hạn phiếu thuê thành công", data: updated });
